...
for i, s in enumerate(songs.iterrows()):
    # skip duplicates
    if s[1].song in map(lambda f: f["song"], pkl_feats):
    ...
    try:
        ...
        ft = sp.audio_features(tracks=[track_id])[0]
        logger.debug("%s Feature: %s" % (s[1].song, ft))
        ...
        ft.update({"artist": s[1].artist, "song": s[1].song, "popularity": popl, "artist_followers": art_followers["followers"]["total"], "release_date": song_date})
        logger.debug("%s Updated: %s" % (s[1].song, ft))
        ...
        if str(e) == "The access token expired":
            # request new token
            logger.critical("Token Expired! Requesting new token...")
            token = util.prompt_for_user_token("Giancarlo Perrone",
                                   "user-library-read",
                                   client_id=client_id,
                                   client_secret=client_secret,
            ...
            # re-instantiate
            sp = spotipy.Spotify(auth=token)
            logger.critical("Successfully authenticated.")
